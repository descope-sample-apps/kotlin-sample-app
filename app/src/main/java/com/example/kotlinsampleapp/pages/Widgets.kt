...
@Composable
fun OTP(
    otpState: TextFieldState = remember { OTPState() },
    imeAction: ImeAction = ImeAction.Next,
    onImeAction: () -> Unit = {}
) {
    OutlinedTextField(
        ...
        modifier = Modifier
            .fillMaxWidth()
            .onFocusChanged { focusState ->
                otpState.onFocusChange(focusState.isFocused)
                if (!focusState.isFocused) {
                    otpState.enableShowErrors()
                }
            },
        textStyle = MaterialTheme.typography.bodyMedium,
        isError = otpState.showErrors(),
        keyboardOptions = KeyboardOptions.Default.copy(
            imeAction = imeAction,
    ...
    otpState.getError()?.let { error -> TextFieldError(textError = error) }
}
